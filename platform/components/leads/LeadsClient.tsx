...
export default function LeadsClient({ initialLeads }: { initialLeads: Lead[] }) {
  ...
  const allCount = useMemo(
    () => initialLeads.filter((l) => !isDncStage(l.stage)).length,
    [initialLeads],
  );
  ...
  const openLead = openLeadId ? (initialLeads.find((l) => l.id === openLeadId) ?? null) : null;
  ...
  function moveToDnc() {
    // No write layer yet — the table is read-only for v1; this logs the intended
  ...
  return (
    <div className={styles.wrap}>
      <div className={`${styles.tableCol} scry`}>
        <FunnelStats leads={initialLeads} />
